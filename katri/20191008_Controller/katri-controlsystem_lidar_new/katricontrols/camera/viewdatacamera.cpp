...
void viewdatacamera::initDataUI(bool isDefault, QMap<int, QString> mapDataSource){
    ...
    for(int i = 0; i< this->mapDataIDCameraIPAddress.size(); i++){
        ...
        if(i == 0 || i == 1){
            setDefaultView(isDefault, ":/img/image/1-Camera.png");
        }
        else if( i == 2){
            setDefaultView(isDefault, ":/img/image/2-Camera.png");
        }
        else{
            setDefaultView(isDefault, ":/img/image/3-Camera.png");
        }
    }
}
void viewdatacamera::setDefaultView(bool isDefault, QString path)
{
    if(isDefault == true){
        video->setbackground("");
    }
