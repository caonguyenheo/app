...
void MainWindow::setupMediator()
{
    ...
    for(int i = 0; i < mapControl.keys().size(); i++)
    {
        ...
        for(int j = 0; j < listButton.size(); j++){
            Device *device = new ConcreteDevice( mediator, listButton.at(j)->getID() , key);
    ...
    connect(mediator,&ConcreteMediator::signalStatusButton,this,&MainWindow::slotUpdateStatusButton);
#endif
}
...
void MainWindow::setupUi()
{
    ...
    m_viewerall = new record(this);
    m_cbbox = new checkedbox(this);
    QHBoxLayout *layout = qobject_cast<QHBoxLayout*>(this->layout());
    layout->setSpacing(0);
    layout->setMargin(0);
    layout->addWidget(m_viewerall);

    //check checkbox is ischecked
    QList<QCheckBox*> getlist = m_viewerall->getListCheckbox();
    Q_FOREACH(QCheckBox *cb_box, getlist) {
        m_viewerall->clickcheckbox(cb_box, this, SLOT(slotcheckedlickchecbox()));
    }
    ...
    m_viewer->viewCameraConnection(this, SLOT(slotViewCameraClicked()));
    ...
    m_viewCamera->recordAllConnection(this, SLOT(slotRecormượndAllBtnClickedviewcamera(bool)));
...
//    m_fullscreen->setVisible(false);
    m_viewerall->setVisible(true);

    this->layout()->setMenuBar(m_menubar);
    this->layout()->menuBar()->installEventFilter(this);

    //Init state button
//    this->m_record->UpdateStatusButton(INIT);

}

void MainWindow::slotcheckedlickchecbox()
{
    QCheckBox *cbsender = dynamic_cast<QCheckBox*>(this->sender());
    if(cbsender) {
        if(cbsender->objectName().toLower().trimmed().contains("cb_front_lidar")) {
            qDebug()<<"cb_front_lidar";
            m_cbbox->getcheckedbox(FRONT_LIDAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_top_lidar")) {
            qDebug()<<"cb_top_lidar";
            m_cbbox->getcheckedbox(TOP_LIDAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_rear_lidar")) {
            qDebug()<<"cb_rear_lidar";
            m_cbbox->getcheckedbox(REAR_LIDAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_can_serial")) {
            qDebug()<<"cb_can_serial";
            m_cbbox->getcheckedbox(CAN_SERIAL);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_mobileye_serial")) {
            qDebug()<<"cb_mobileye_serial";
            m_cbbox->getcheckedbox(MOBILEYE_SERIAL);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_weather_serial")) {
            qDebug()<<"cb_weather_serial";
            m_cbbox->getcheckedbox(WEATHE_SERIAL);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_frontcenter_radar")) {
            qDebug()<<"cb_frontcenter_radar";
            m_cbbox->getcheckedbox(FRONTCENTER_RADAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_frontleft_radar")) {
            qDebug()<<"cb_frontleft_radar";
            m_cbbox->getcheckedbox(FRONTLEFT_RADAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_frontright_radar")) {
            qDebug()<<"cb_frontright_radar";
            m_cbbox->getcheckedbox(FRONTRIGHT_RADAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_rearleft_radar")) {
            qDebug()<<"cb_rearleft_radar";
            m_cbbox->getcheckedbox(REARLEFT_RADAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_rearright_radar")) {
            qDebug()<<"cb_rearright_radar";
            m_cbbox->getcheckedbox(REARRIGHT_RADAR);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_front_fhd")) {
            qDebug()<<"cb_front_fhd";
            m_cbbox->getcheckedbox(FRONT_FHD);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_left_fhd")) {
            qDebug()<<"cb_left_fhd";
            m_cbbox->getcheckedbox(LEFT_FHD);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_right_fhd")) {
            qDebug()<<"cb_right_fhd";
            m_cbbox->getcheckedbox(RIGHT_FHD);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_rear_fhd")) {
            qDebug()<<"cb_rear_fhd";
            m_cbbox->getcheckedbox(REAR_FHD);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_front_svm")) {
            qDebug()<<"cb_front_svm";
            m_cbbox->getcheckedbox(FRONT_SVM);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_left_svm")) {
            qDebug()<<"cb_left_svm";
            m_cbbox->getcheckedbox(LEFT_SVM);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_right_svm")) {
            qDebug()<<"cb_right_svm";
            m_cbbox->getcheckedbox(RIGHT_SVM);
        }
        else if (cbsender->objectName().toLower().trimmed().contains("cb_rear_svm")) {
            qDebug()<<"cb_rear_svm";
            m_cbbox->getcheckedbox(REAR_SVM);
        }
    }
}
...
#if (ENABLE_UI_OLD == 1)
void MainWindow::slotUpdateStatusButton(int status)
{
    // Update status for button record,stop and relay
    if (status == DETECT_RECORD) {
        this->m_record->UpdateStatusButton(DETECT_RECORD);
    }
    else if (status == DETECT_STOP) {
        this->m_record->UpdateStatusButton(DETECT_STOP);
    }
    else if (status == RECORD_ALL) {
        this->m_record->UpdateStatusButton(RECORD_ALL);
    }
}
...
{
    this->m_viewer->recordAllActivation(active);
    this->m_viewer->setStatus(active);
    this->m_record->updateStatusRecordButton(active);
    if (active==true) {
        this->m_record->UpdateStatusButton(RECORD_ALL);
    }
    else {
        this->m_record->setRelayEnabled(true);
    }
}
...
void MainWindow::slotRelayAllBtnClicked(bool active)
{
    if (m_viewer->isRecording() == false && active == true) {
        ...
        this->m_record->UpdateStatusButton(RELAYING);
        sendRelayCommand();
}
    else {
        this->m_record->UpdateStatusButton(RELAY_STOP);
    }
}

void MainWindow::slotStopAllBtnClicked(bool active)
{
    this->m_record->updateStatusStopButton(active);
    if (active==true) {
        this->m_record->UpdateStatusButton(STOP_ALL);
    }
    else {
        this->m_record->setRelayEnabled(!active);
    }
}
...
void MainWindow::slotViewCameraClicked()
{
    m_viewer->setVisible(false);
    m_record->setVisible(false);
    m_viewChart->setVisible(false);
    this->setMinimumSize(1200QRadioButton *rdSender = dynamic_cast<QRadioButton*>(this->sender());, 967);
    this->resize(1200, 967);
...
void MainWindow::slotSendCommand(Command::Type deviceType, int deviceId, bool recordStatus, bool isNewDS)
{
    ...
    else if (recordStatus == QRadioButton *rdSender = dynamic_cast<QRadioButton*>(this->sender());true && isNewDS == false) {
...
void MainWindow::slotReceiveVideoFrame(int deviceType, int deviceId, QByteArray data)
{
    ...
    if (deviceId >= 0 && deviceId <= 3) {
        ...
        m_viewCamera->setDataViewHD(Command::Type(deviceType), deviceId, data);
    }
    else if (deviceId >= 4 && deviceId <= 7) {
        qDebug() << "Send frame data to FHD camera " << deviceId;
        m_viewCamera->setDataViewFullHD(Command::Type(deviceType), deviceId, data);
    }
    else {
