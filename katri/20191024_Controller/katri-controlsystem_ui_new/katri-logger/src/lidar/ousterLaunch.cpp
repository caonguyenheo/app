#include "ousterLaunch.h"

#if ENABLE_ROS_VERSION == 1
OusterLaunch::OusterLaunch(QObject *parent)
    : QThread(parent)
{
    _logger = LoggerManager::getAppLogger();
}
OusterLaunch::~OusterLaunch()
{
    if (_process)
        delete _process;
}
void OusterLaunch::run()
{
    QStringList arguments;
    arguments << "-c";
    arguments << "./ouster.launch.sh";
    ...
    connect(_process,
            ...
            [=](QProcess::ProcessError error) {
                ...
                if (_logger)
                    _logger->debug("Ouster process occurred error: {}", msg.toStdString());
            });
    ...
    connect(_process,
            ...
            [=](int exitCode, QProcess::ExitStatus exitStatus) {
                ...
                if (_logger)
                    _logger->debug(
                        "Ouster process finished with exit code: {} and exist status: {}.",
                        exitCode,
                        msg.toStdString());
            });
...
//void OusterLaunch::captureDataOutput()
//{
