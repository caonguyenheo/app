...
bool Command::isPingFromControllerNode() const
{
    if (_action == Action::PING && _type == Type::CONTROLLER
        && _id == AppConfig::instance().getControllerNodeName().toStdString()) {
        return true;
    }
    return false;
}

bool Command::isAckFromControllerNode() const
{
    if (_action == Action::ACK && _type == Type::LOGGER
...
bool Command::isRelayCommand() const
{
