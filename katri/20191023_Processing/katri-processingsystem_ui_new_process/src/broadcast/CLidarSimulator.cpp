#include "CLidarSimulator.h"

#include <QDebug>
//#include "viewerpanel.h"
#include <cmath>

const QString LIDAR_PREFIX_0 = "vlp16_0";
const QString LIDAR_PREFIX_1 = "vlp16_1";
const QString LIDAR_PREFIX_2 = "os1_0";

CLidarSimulator::CLidarSimulator(IMediator *mediator, int id)
{
    p_mediator = mediator;
    m_id = id;
...
void CLidarSimulator::receiveMessage(EventType eventType, EventObject *eventObject)
{
    switch (eventType) {
...
void CLidarSimulator::sendMessage(EventType type, EventObject *eventObject)
{
    p_mediator->broadcast(type, eventObject);
}

QString CLidarSimulator::getSourcePrefix(int id)
{
    QString prefix = "";
    switch (id) {
    case 1:
        prefix = LIDAR_PREFIX_1;
        break;
    case 2:
        prefix = LIDAR_PREFIX_2;
        break;
    default:
        prefix = LIDAR_PREFIX_0;
        break;
    }
    return prefix;
}

QString CLidarSimulator::getBasePath(QString kittiPath)
{
...
void CLidarSimulator::loadTimestamp(EventObject *eventObject)
{
    ...
    while (it.hasNext()) {
        ...
        if (f.open(QIODevice::ReadOnly)) {
            ...
            while (!stream.atEnd()) {
                ...
                // Build "xxxxxxxxxx_0000000000.txt" image file name
                QString imgName = tsIndex + QString("%1").arg(imgIndex, 10, 10, QLatin1Char('0')) + ".txt";
...
QMap<int,QString> *CLidarSimulator::buildLocalTimeline(QMap<int,QString> &ts_map, QString first, QString last)
{
...
void CLidarSimulator::loadFrameData(EventObject *eventObject)
{
    if (localTimeline) {
...
void CLidarSimulator::slotFinishedRender()
{
