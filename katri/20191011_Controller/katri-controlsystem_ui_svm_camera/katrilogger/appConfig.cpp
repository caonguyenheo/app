...
QString AppConfig::getDeviceName(QString &ipDevice)
{
    return _portTable.key(ipDevice, "");
}

int AppConfig::getLoggerIdFromIp(const QString &ipAddr)
{
    QString key = _portTable.key(ipAddr);
    if (key.contains("_")) {
        key = key.mid(key.lastIndexOf("_")+1);
    }
    return key.toInt();
}

QList<QString> AppConfig::getListDeviceName(QString &ipDevice)
{
    return _portTable.keys(ipDevice);
}
