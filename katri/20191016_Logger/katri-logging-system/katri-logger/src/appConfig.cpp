...
void AppConfig::reload()
{
    ...
    _ipserver= _config.value("ipserver", "").toString();
    _portserver = _config.value("portserver","").toString();
    _ipclient= _config.value("ipclient", "").toString();

    QString keepData = _config.value("keep_kitti_data", "false").toString();
    _keepKittiData = keepData == "true" ? true : false;
    _deviceName = _config.value("devicename", "").toString();
}
...
QString AppConfig::getIpServer() const
{
    return _ipserver;
}

QString AppConfig::getPortServer() const
{
    return _portserver;
}

QString AppConfig::getIpClient() const
{
    return _ipclient;
}

QString AppConfig::getDeviceName() const
{
    return _deviceName;
}
